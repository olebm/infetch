...
- **Server-side org filters.** Server Actions and API routes that touch user data run on a postgres-superuser connection (via Supavisor) which bypasses RLS — they therefore filter by `organization_id` manually in every query. All source files use `createScopedSql(orgId)` (org-scoped queries) or `unsafeGlobalSql` (explicitly cross-org, with an in-code comment justifying why). The bare `sql` import from `@/lib/db/client` is blocked by ESLint `no-restricted-imports` — the `SQL_CLIENT_ALLOWLIST` in `eslint.config.mjs` is now limited to test fixtures only, making the audit surface for raw SQL trivially small.

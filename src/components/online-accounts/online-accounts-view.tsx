    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <p className="text-sm text-muted">
          Wir holen Rechnungen automatisch aus diesen Konten ab. Du musst dich pro Konto nur einmal anmelden.
        </p>
      ...
      {accounts.length === 0 ? (
        <div className="rounded border border-dashed border-line bg-surface px-6 py-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-muted">
            Verbinde dein erstes Online-Konto — wir holen ab dann automatisch alle Rechnungen ab und legen sie in
            deinen Posteingang.
          </p>
        </div>
      ) : (
...
function deriveStatus(
  ...
  if (lastRun.status === "login_required") return { label: "Login abgelaufen", color: "text-orange-700" };
  ...
  if (lastRun.status === "captcha") return { label: "Bitte einmal manuell anmelden", color: "text-orange-700" };

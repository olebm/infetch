...
type VendorRow = {
  ...
  vendorName: string;
  canonicalKey: string;
  hasDomainAlias: string;
  hasExactAlias: string;
};
 ...
 */
export async function backfillDomainAliases(): Promise<AliasBackfillResult> {
  ...
  const vendors = await sql<VendorRow[]>`
    ...
    HAVING SUM(CASE WHEN a.match_type = 'domain' THEN 1 ELSE 0 END) = 0
  `;
  ...
  for (const vendor of vendors) {
    ...
    const domains = await sql<Array<{ fromAddress: string; uses: string }>>`
      ...
      WHERE i.vendor_id = ${vendor.vendorId}
    ...
    const existing = await sql`
      ...
      WHERE vendor_id = ${vendor.vendorId} AND alias = ${domain} AND match_type = 'domain'
      LIMIT 1
    `;
    ...
    const inserted = await sql`
      ...
      VALUES (${vendor.vendorId}, ${domain}, 'domain', 50)
    ...
    if (inserted.count > 0) {
      ...
      await recordSyncEvent({
        ...
        metadata: { alias: domain, basisUses: Number(domains[0].uses) },
      });
    }
  }

 ...
 * Jeder Knoten bekommt eine ID ("el-1", "el-2", ...). Passwoerter werden NIE
 * mitgeschickt.
 */
...
export async function snapshotTree(page: Page): Promise<SerializedTree> {
  const rawNodes = await page.evaluate((max: number) => {
    type Out = {
      role: string;
      name: string | null;
      value: string | null;
  ...
  rawNodes.forEach((node, idx) => {
    ...
    if (node.value !== null && node.value !== undefined && !node.isPassword) {

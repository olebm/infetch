...
describe.skipIf(!hasDb)(
  "ensureUserProvisioned — soft-deleted leiche recovery (INFETCH-220)",
  () => {
    beforeEach(cleanup);
    afterEach(cleanup);
      ...
      expect(
        Number(
          (
            await sql<
              { c: string }[]
            >`SELECT COUNT(*) c FROM organizations WHERE id = ${LEICHE_ORG_A}`
          )[0].c,
        ),
      ).toBe(1);
      expect(
        Number(
          (
            await sql<
              { c: string }[]
            >`SELECT COUNT(*) c FROM vendors WHERE canonical_key = ${VENDOR_KEY_A}`
          )[0].c,
        ),
      ).toBe(1);
    });
    ...
    it("empty leiche: hard-purges the dead row and provisions fresh", async () => {
      ...
      expect(
        Number(
          (await sql<{ c: string }[]>`SELECT COUNT(*) c FROM users WHERE id = ${LEICHE_B}`)[0].c,
        ),
      ).toBe(0);
      expect(
        Number(
          (
            await sql<
              { c: string }[]
            >`SELECT COUNT(*) c FROM organizations WHERE id = ${LEICHE_ORG_B}`
          )[0].c,
        ),
      ).toBe(0);
